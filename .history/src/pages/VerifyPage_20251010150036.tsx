...
export default function VerifyPage() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      try {
        // Hem hash (#) hem query (?) parametrelerini alalım
        const hashParams = new URLSearchParams(window.location.hash.replace("#", ""));
        const queryParams = new URLSearchParams(window.location.search);

        // Supabase bazen hata parametresi gönderir
        ...
        // Access ve refresh token sadece magic link (type=magic_link) ile gelir
        const access_token =
          hashParams.get("access_token") || queryParams.get("access_token");
        const refresh_token =
          hashParams.get("refresh_token") || queryParams.get("refresh_token");

        // Eğer token varsa (örneğin magic link)
        if (access_token && refresh_token) {
          ...
          console.log("✅ Kullanıcı oturumu oluşturuldu:", user.email);

          // Kullanıcı tablosunda verified güncelle
          await supabase
            .from("users")
            .update({
              verified: true,
          ...
          // Eğer signup e-postası ile geldiyse (type=signup)
          // Supabase backend’de zaten doğrulamayı yapar
          console.log("✅ Supabase signup doğrulaması tamamlandı (tokensiz).");
        }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-white text-center">
      <div className="bg-white p-10 rounded-2xl shadow-lg max-w-md border border-red-100">
        ...
        <p className="text-gray-600 mb-6">
          Bağlantı geçersiz veya süresi dolmuş olabilir. Lütfen yeni bir bağlantı isteyin.
        </p>
        <a
          href="/"
          className="inline-block bg-teal-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-teal-700 transition"
