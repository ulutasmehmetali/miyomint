...
export default function GuestCheckoutPage({
  ...
  // 🛒 Supabase’ten sepet yükle (tamamen Supabase)
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        setCart([]); // fallback olarak boş sepet
      }
    };
    fetchCart();
  }, []);
  ...
  // 💳 Misafir sipariş kaydetme (tamamen Supabase)
  const handlePayment = async () => {
    setLoading(true);
    try {
      const orderId = crypto.randomUUID?.() || Math.random().toString(36).slice(2, 10);

      // 1️⃣ guest_orders tablosuna kayıt
      const orderData = {
        id: orderId,
        full_name: form.name,
        email: form.email,
      ...
      // 2️⃣ guest_order_items tablosuna ürünleri ekle
      if (cart.length > 0) {
        await Promise.all(
          cart.map(async (item) => {
            const itemData = {
            ...
            await supabaseService.addGuestOrderItem(itemData);
          })
        );
      }

      // 3️⃣ Supabase’de sepeti temizlemek istiyorsan (opsiyonel):
      // await supabase.from("guest_order_items").delete().neq("order_id", orderId);

      setCart([]);
      setStep(3);

      // 4️⃣ Bildirim (isteğe bağlı e-posta gönderme fonksiyonu)
