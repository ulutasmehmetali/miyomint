    ...
    const verify = async () => {
      try {
        ...
        if (!access_token || !refresh_token) {
          console.warn("Tokenlar bulunamadı:", hash);
          setStatus("error");
          return;
        }
        ...
        if (sessionError || !data.session) {
          console.error("Oturum kurulamadı:", sessionError);
          setStatus("error");
          return;
        }

        const user = data.session.user;
        console.log("✅ Email verified and user session active:", user.email);

        // ✅ Profil tablosunda e-posta doğrulamasını güncelle
        await supabase
          .from("profiles")
          .update({
            email_verified: true,
        ...
        // 🎉 Toast bildirimi
        toast.success("E-posta başarıyla doğrulandı! Oturum açılıyor...", {
          style: { borderRadius: "10px", background: "#333", color: "#fff" },
          duration: 2500,
        });

        // ✅ 2.5 sn sonra ana sayfaya yönlendir
        setTimeout(() => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-white text-center animate-fade-in">
        <div className="bg-white p-10 rounded-2xl shadow-lg max-w-md border border-green-100">
          ...
          <p className="text-gray-600 mb-4">
            E-posta adresiniz doğrulandı, oturumunuz otomatik olarak açıldı.
          </p>
