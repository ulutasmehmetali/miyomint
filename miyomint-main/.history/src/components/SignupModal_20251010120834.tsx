...
export default function SignupModal({ isOpen, onClose, onSwitchToLogin }: SignupModalProps) {
  ...
  // 🔒 Şifre kuralı: en az 6 karakter, 1 büyük harf, 1 rakam
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Supabase üzerinden kayıt ve doğrulama maili gönder
      const { data, error } = await supabase.auth.signUp({
        email,
        password,
        options: {
          data: { full_name: fullName },
          emailRedirectTo: "https://miyomint.com.tr/#verify", // doğrulama sonrası yönlendirme
        },
      });

      if (error) throw error;

      // Opsiyonel: users tablosuna kaydet
      if (data.user) {
