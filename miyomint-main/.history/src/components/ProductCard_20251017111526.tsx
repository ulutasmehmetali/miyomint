...
export default function ProductCard({ product, onAddToCart, onBuyNow }: ProductCardProps) {
  ...
  const handleAddToCart = async () => {
    try {
      await onAddToCart(product); // App içindeki handleAddToCart tetikleniyor
  ...
  const handleBuyNow = async () => {
    try {
      await onBuyNow(product); // App içindeki handleBuyNow tetikleniyor
      toast.success("Satın alma işlemi başlatılıyor 🛍️", {
        style: { borderRadius: "10px", background: "#333", color: "#fff" },
      });
    } catch (err) {
      console.error("Satın alma hatası:", err);
      toast.error("Satın alma başlatılamadı ❌");
    }
  };
