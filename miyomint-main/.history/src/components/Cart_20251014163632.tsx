...
export default function Cart({
  ...
  // ✅ Animasyon için state kontrolü
  useEffect(() => {
    if (isOpen) {
      setVisible(true);
      document.body.style.overflow = 'hidden'; // scroll engelle
    } else {
      const timeout = setTimeout(() => setVisible(false), 300); // animasyon bitince gizle
  ...
  // ✅ Siparişi tamamla tıklanınca önce sepeti kapat, sonra checkout çalışsın
  const handleCheckoutClick = () => {
    onClose(); // önce sepeti kapat
    setTimeout(() => {
      onCheckout(); // 300ms sonra yönlendirme çalışsın
    }, 300);
  };
      ...
      >
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-200 p-6 bg-gray-50">
            ...
            {/* ✅ Burada handleCheckoutClick kullanıyoruz */}
            <button
