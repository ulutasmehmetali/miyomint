...
export default function Cart({ isOpen, onClose, onCheckout }: CartProps) {
  ...
  // 🧩 Supabase’ten sepet yükle (giriş durumuna göre)
  useEffect(() => {
  ...
  const handleUpdateQuantity = async (itemId: string, newQty: number) => {
    ...
    try {
      if (user) {
        await supabaseService.updateCartItem(itemId, newQty);
      } else {
  ...
  // 💳 Checkout butonu — önce panel kapanır, sonra yönlendirme yapılır
