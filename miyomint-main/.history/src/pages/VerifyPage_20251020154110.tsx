...
export default function VerifyPage() {
  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        console.log("🚀 E-posta doğrulama süreci başladı...");
        ...
        // 🔹 PKCE doğrulama (Supabase 2.x)
        if (!access_token && !refresh_token) {
          console.log("🔐 PKCE akışı algılandı (token yok, Supabase yönetecek).");
          await new Promise((r) => setTimeout(r, 1500)); // propagation delay
        } else {
          // 🔑 Eski token bazlı doğrulama (geri uyumluluk)
          const access = access_token ?? "";
          const refresh = refresh_token ?? "";

          if (!access || !refresh) {
            console.warn("⚠️ access_token veya refresh_token eksik!");
            setStatus("error");
            return;
          }
          ...
          await new Promise((r) => setTimeout(r, 1000)); // propagation fix
        }
        ...
        // ✅ Profil tablosunu kontrol et / oluştur
        try {
        ...
        // 🎉 Başarılı
        console.log("🎉 Doğrulama tamamlandı:", user.email);
