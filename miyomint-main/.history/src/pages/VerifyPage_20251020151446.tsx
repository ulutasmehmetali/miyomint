  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        ...
        // ⏳ 2 saniye bekle (bazı token gecikmeleri için)
        console.log("⏳ getSession öncesi 2 saniye bekleniyor...");
        await new Promise((r) => setTimeout(r, 2000));

        console.log("🚀 getSession çağrısı başlatıldı...");
        const {
          data: { session },
          error: getSessionError,
        } = await supabase.auth.getSession();
        console.log("📦 getSession tamamlandı:", session, getSessionError);

        if (getSessionError) {
          console.error("❌ getSession hatası:", getSessionError.message);
        }

        console.log("🪪 Şimdi getUser() çağrılıyor...");
