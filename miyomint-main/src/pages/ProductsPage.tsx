...
interface ProductsPageProps {
  onAddToCart: (product: Product) => Promise<void>; // 🔹 async olarak düzeltildi
  onBuyNow: (product: Product) => Promise<void>; // 🔹 async olarak düzeltildi
}
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50 py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
        <header className="text-center space-y-6">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-100 text-teal-700 text-sm font-semibold mx-auto">
          ...
          <p className="max-w-2xl mx-auto text-gray-600 text-base sm:text-lg">
            Her paket, kedinizin ihtiyaçlarına göre optimize edildi. Sepetinizi oluşturun, rahat bir nefes alın.
          </p>
        </header>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {sellingPoints.map((item) => (
            <div
              key={item.title}
              className="rounded-2xl bg-white border border-gray-100 shadow-sm p-5 sm:p-6 flex flex-col gap-3 lg:hover:shadow-lg transition-shadow"
            >
        ...
        <section className="space-y-8">
          <div className="text-center space-y-2">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Paket seçenekleri</h2>
